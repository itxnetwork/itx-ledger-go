...
some [good first issues](https://github.com/itxnetwork/itx/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Itx, please create
an [ADR](https://github.com/itxnetwork/itx/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/itxnetwork/itx`, the code should never exist
at `$GOPATH/src/github.com/itxnetwork/itx`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/itxnetwork/itx`, and do all the work there.
